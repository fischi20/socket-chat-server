...
function splitStringEqually(str: string, n: number) {
...
function computeXOR(a: string, b: string) {
...
export function encodeString(binary: string): string[] {
...
export function generateSubKeys(binKey: string) {
    if (!isBinary(binKey)) {
        binKey = strToBin(binKey)
    }
    const { PC1, PC2, KEY_SHIFTS } = matrices;
    const keys = [];
    let binKey_PC1: string = '';
...
export function encryptDES(keys: string[], ip: string) {
    let cyphertext = '';
...
export function decryptDES(keys: string[], message: string) {
...
export class DES {
    ...
    generateKeys(key: string = generateKey()) {
     ...
     * @param cyphertext cyphertext to be encoded 
     */
    decryptFull(cyphertext: string) {
